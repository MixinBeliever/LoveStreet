<template>
    <div class="mystreet">
        <commonStreet :itemlist="this.$store.state.ladyStreetItemList" 
...
export default {
    data(){
        return {
            itemlist: this.$store.state.ladyStreetItemList,
            curpage: 1,
            loading: false,
        }
    },
    mounted(){
        if(this.$store.state.ladyStreetItemList.length == 0){
            store.dispatch("ladyStreet");
            console.log('dispatch了')
        }else{

        }
    },
    updated(){
        console.log('更新周中',this.itemlist)
    },
    methods: {
        loadMore() {
            console.log('到底了')
